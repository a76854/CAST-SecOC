"""Simplified CAN FD transmission-time and queueing model."""
import random
import time
...
class CanFDBus:
    ...
    def __init__(self, load: float = 0.30,
        ...
        if not 0.0 <= load < 1.0:
            raise ValueError("bus load must be in the range [0, 1)")
    ...
    def _queue_delay(self, tx_time: float) -> float:
        ...
        At high load → nonlinear growth in this simplified model.
        """
        if self.load <= 0.01:
            return 0.0
        ...
        return self.rng.expovariate(1.0 / max(mean_wait, 1e-9))
